...
// DefaultSSMServiceFactory is the default factory that creates SSM services.
func DefaultSSMServiceFactory(scope cloud.ClusterScoper) *ssm.Service {
...
type NodeadmConfigReconciler struct {
	...
	// SSMServiceFactory creates SSM services. If nil, DefaultSSMServiceFactory is used.
...
func (r *NodeadmConfigReconciler) reconcileHybridNode(ctx context.Context, cluster *clusterv1.Cluster, config *eksbootstrapv1.NodeadmConfig, configOwner *bsutil.ConfigOwner) (ctrl.Result, error) {
	...
	// Skip if already ready (similar to joinWorker for Machines)
	if config.Status.DataSecretName != nil && configOwner.GetKind() == "Machine" {
		secretKey := client.ObjectKey{Namespace: config.Namespace, Name: *config.Status.DataSecretName}
		existingSecret := &corev1.Secret{}
		err := r.Client.Get(ctx, secretKey, existingSecret)
	...
	activationID, activationCode, err := r.getOrCreateSSMActivation(ctx, cluster, controlPlane, config)
	if err != nil {
	...
	hybridInput := &userdata.HybridNodeadmInput{
		ClusterName:        controlPlane.Spec.EKSClusterName,
	...
	if config.Spec.Kubelet != nil {
		hybridInput.KubeletFlags = config.Spec.Kubelet.Flags
		if config.Spec.Kubelet.Config != nil {
			hybridInput.KubeletConfig = config.Spec.Kubelet.Config
		}
	}

	// Add containerd configuration
	if config.Spec.Containerd != nil {
		hybridInput.ContainerdConfig = config.Spec.Containerd.Config
	}
	...
	// Generate userdata
	userDataScript, err := userdata.NewHybridNodeadmUserdata(hybridInput)
	if err != nil {
...
func (r *NodeadmConfigReconciler) getOrCreateSSMActivation(ctx context.Context, cluster *clusterv1.Cluster, controlPlane *ekscontrolplanev1.AWSManagedControlPlane, config *eksbootstrapv1.NodeadmConfig) (string, string, error) {
	log := logger.FromContext(ctx)
	...
	// Option 1: Use referenced activation secret
	...
	expirationDays := int32(7)
	if ssmOpts.ActivationConfig.ExpirationDays != nil {
		expirationDays = *ssmOpts.ActivationConfig.ExpirationDays
	}
	...
	params := &ssm.HybridActivationParams{
		IAMRoleARN:        ssmOpts.ActivationConfig.IAMRoleARN,
...
func (r *NodeadmConfigReconciler) reconcileDelete(ctx context.Context, cluster *clusterv1.Cluster, config *eksbootstrapv1.NodeadmConfig) (ctrl.Result, error) {
	...
	if config.Status.SSMActivation != nil && config.Status.SSMActivation.ActivationID != nil {
		...
		if err := r.Get(ctx, client.ObjectKey{Name: cluster.Spec.ControlPlaneRef.Name, Namespace: cluster.Namespace}, controlPlane); err != nil {
			...
			log.Info("Control plane not found, skipping SSM activation deletion")
		} else {
			...
			if err := ssmService.DeleteHybridActivation(ctx, activationID); err != nil {
				log.Error(err, "Failed to delete SSM activation")
...
func (r *NodeadmConfigReconciler) getSSMService(ctx context.Context, cluster *clusterv1.Cluster, controlPlane *ekscontrolplanev1.AWSManagedControlPlane) (*ssm.Service, error) {
	...
	factory := r.SSMServiceFactory
	if factory == nil {
		factory = DefaultSSMServiceFactory
	}

	return factory(managedScope), nil
}
